 ...
 *
 */
class NaptchaComponent extends Component {
	...
	public function create() {
		...
		$this->Session->write('naptcha.' . $id, array());
		return $id;
	}
	...
	public function image($id) {
		...
		if (is_array($captchaInfo)) {
			...
			/** Text insertion */
			$captchaInfo['time'] = time();
			$captchaInfo['text'] = $this->GetRandomCaptchaText();
			$this->Session->write('naptcha.' . $id, $captchaInfo);
	...
	public function validate($id, $text) {
		...
		if (!empty($captchaInfo)) {
			...
			elseif ($captchaInfo['text'] != $text) {
				$this->errorCode = 'NOTMATCHED';
				$this->errorMessage = $this->errorMap[$this->errorCode];
				return false;
			}
			return true;
